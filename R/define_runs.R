...
#'
#' @description if it doesn't exist download it to
#'
#'   <parameters$data_path$scratch_dir>/sra_meta/SRAmetadb.sqlite
#'
#' @param parameters list having element data_paths$scratch_dir
#' @param verbose print verbose output
#' @return dtabase connection to the sra metadata database
#' @export
get_sra_connection <- function(parameters, verbose = TRUE) {
    ...
    DBI::dbConnect(RSQLite::SQLite(), sra_fname)
}
...
retrieve_sra_runs <- function(
    ...
    if (!("study_accession" %in% names(curated_studies))) {
        cat("ERROR: curated_studies must be a data.frame with ",
            "a column column 'study_accession'\n", sep = "")
    }
    ...
    if (nrow(missing_studies) > 0) {
        cat("WARNING: ", nrow(missing_studies), " studies could not be found ",
            "in the SRA database:\n", sep = "")
        print(missing_studies)
    }

    if (verbose) {
        cat("Retrieving runs for ", nrow(curated_studies),
            " curated studies ...\n", sep = "")
    }
    runs <- sra_con |>
        dplyr::tbl("curated_studies") |>
