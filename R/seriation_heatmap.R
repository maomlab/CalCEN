...
#'
#' @description Given a matrix x, his makes a dendrogram where the columns are
#'   sorted to minimize the row-to-row difference the sorting is done relative
#'   to the ref_x, if it is supplied
seriation_heatmap <- function (
    ...
    if (!is.null(ref_x)) {
        ...
        if (any(class(ref_x) == "dist")) {
            ...
            if (attr(ref_x, "Size") != nrow(x)) {
                cat(
                    "ERROR: the size of the provided ref_x is ",
                    attr(ref_x, "Size"), " while nrow(x)=", nrow(x), "\n", sep = "")
            }
            dist_row <- ref_x
            dist_col <- t(ref_x)
        } else {
            ...
            if (nrow(ref_x) != nrow(x)) {
                cat(
                    "ERROR: nrow(ref_x) = ", nrow(ref_x), " !=  nrow(x) = ",
                    nrow(x), "\n", sep = "")
            }
            dist_row <- dist(ref_x)
        ...
        if (verbose) {
            cat(
                "Computing seriation with method '", seriate_method, "'\n",
                sep = "")
        }
        o_row <- seriation::seriate(
            dist_row, method = seriate_method, control = seriate_control)[[1]]
        Rowv <- o_row |> seriation::get_order()
        o_col <- seriation::seriate(
            dist_col, method = seriate_method, control = seriate_control)[[1]]
    ...
    if (is.null(color_scale)) {
        if (any(x < 0, na.rm = TRUE)) {
            if (verbose) {
                cat(
                    "Scores have values that are less than zero, ",
                    "so using blue-red colors ...\n")
            }
            args$col <- seriation::bluered(n=100, bias=1)
        } else {
            if (verbose) {
                cat(
                    "Scores are all greater than zero using ",
                    "white-black colors ...\n")
            }
    ...
    pdf(fname, height = height, width = width)
        suppressWarnings(do.call(gplots::heatmap.2, args))
    dev.off()
}
