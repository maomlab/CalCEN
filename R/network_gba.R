...
#' @param annotation_sets list of go term annotations to predict
#' @param networks list of networks used for prediction
#' @param min filter terms requiring a minimum number of annotations (Default: 20)
#' @param max filter terms requiring a maximum number of annotations (Default: 1000)
#' @param nfold (Default 3)
#' @param degrees (Default 1:length(networks))
#' @param verbose (Default FALSE)
#' @return data.frame with columns
#'    anno_id
#'    auroc_mean
#'    auroc_std
#'    degree_null_auroc_mean
#'    degree_null_auroc_std
#'@export
network_gba <- function (
    genes,
    annotation_sets,
    networks,
		min = 20,
    ...
    if (is.data.frame(networks) | is.matrix(networks)) {
        ...
        if (!all(degrees <= length(networks) & degrees >= 0)) {
						stop(
								"Degrees must be a list of numbers in [1, ", length(networks), "] ",
								"but instead it is ", paste0(degrees, collapse = ", "), "\n", sep = "")
        }
    }
		for (i in 1:length(networks)) {
				if (!all(genes == rownames(networks[i]))) {
						stop(
								"The row names for the ", i, "th network, ", names(networks)[i], ", ",
								"does not match the names in 'genes'\n", sep = "")
        }
        if (!all(genes == colnames(networks[i]))) {
						stop(
								"The column names for the ", i, "th network, ", names(networks)[i], ", ",
								"does not match the names in 'genes'\n", sep = "")
        }
    }
    ...
    for (i in 1:length(annotation_sets)) {
        if (!all(genes == rownames(annotation_sets[i]))) {
                stop(
										"The row names for the ", i, "th annotation set, ",
										names(annotation_sets)[i], ", ",
										"does not match the names in 'genes'\n", sep = "")
        }
    }
    ...
    networks %>% purrr::map2(names(.), ., function(network_id, network) {
        if (!all(rownames(network) == genes)) {
						stop(
								"The row names of '{network_id}' network do not match the supplied genes\n" %>%
								glue::glue())
        }
        if (!all(colnames(network) == genes)) {
                stop(
										"The column names of {network_id} do not match the supplied genes\n" %>%
										glue::glue())
        }
    })
    ...
    z <- plyr::ldply(names(annotation_sets), function(anno_id) {
        ...
        plyr::ldply(degrees, function(network_degree) {
                plyr::ldply(
										networks %>% names %>% combn(network_degree, simplify = FALSE),
										function(network_ids) {
												network_id <- paste0(network_ids, collapse="|")
												if (verbose) {
														cat(
																"Computing GBA for '", anno_id, "' ",
																"annotations, using the ",
																network_id, " network: ", sep = "")
												}
												...
												colnames(net) <- genes
												z <- tryCatch({
														gba <- EGAD::run_GBA(net, annotation_set, min = min, max = max, nfold = nfold)
														if (verbose) {
																cat(
