...
save(
		coexp_intra_study_coexp_network_spearman,
		file = paste0(
				"product/coexp_intra_study_coexp_network_spearman_", CalCEN::date_code(), ".Rdata"))

coexp_intra_study_coexp_network_spearman %>%
	as.data.frame() %>%
		readr::write_tsv(
				file = paste0(
						"product/coexp_intra_study_coexp_network_spearman_", CalCEN::date_code(), ".tsv"))
...
save(
		...
		file = paste0("product/coexp_network_full_pearson_", CalCEN::date_code(), ".Rdata"))
coexp_network_full_pearson %>%
	as.data.frame() %>%
		readr::write_tsv(
				file = paste0(
...
save(
		coexp_full, file = "intermediate_data/coexp_full_pearson.Rdata")
...
save(coexp_full_direct_bootstrap_60, file = "intermediate_data/coexp_full_direct_bootstrap_60.Rdata")

# check
results_60 <- coexp_full_direct_bootstrap_60$merge %>%
