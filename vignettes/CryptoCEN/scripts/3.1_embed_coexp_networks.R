...
save(
    coexp_cds,
    file = paste0("intermediate_data/coexp_cds_by_gene_",
...
ggplot2::ggsave(
    filename = paste0(
        "product/figures/coexp_embedding/UMAP_genes_",
...
gene_clusters <- data.frame(
    ...
    cluster_label = coexp_cds@clusters[["UMAP"]]$clusters)
...
old_gene_clusters <- readr::read_tsv(
    file = paste0(
        "product/figures/coexp_embedding/",
        "UMAP_genes_cluster_labels_", CalCEN::date_code(), ".tsv"))
...
coexp_cds@clusters[["UMAP"]]$clusters <-
    old_gene_clusters$cluster_label |> factor()
...
ggplot2::ggsave(
    filename = paste0(
        "product/figures/coexp_embedding/",
        "UMAP_genes_old_clustering", CalCEN::date_code(), ".pdf"),
    width = 6,
    height = 6)
