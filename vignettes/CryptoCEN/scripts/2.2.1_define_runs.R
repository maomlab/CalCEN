...
runs <- runs |>
    dplyr::filter(
        !(study_accession == "SRP015456") |
            (scientific_name |> stringr::str_detect("H99")),
...
runs2 |>
    dplyr::distinct(study_accession, .keep_all = TRUE) |>
    dplyr::left_join(
        runs2 |> dplyr::count(study_accession, sort = TRUE),
        by = "study_accession") |>
    dplyr::filter(n != n_runs)
