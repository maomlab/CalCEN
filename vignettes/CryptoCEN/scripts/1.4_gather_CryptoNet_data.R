...
# Integrated network
# [CryptoNet: a network by integration of all data-type-specific networks
#  (CN-CC,CN-CX,CN-DC,CN-GN,CN-PG,HS-HT,HS-LC,SC-CC,SC-CX,SC-DC,SC-GT,SC-HT,
#   SC-LC,SC-TS)]
...
# sub networks
for (subnetwork_type in
    parameters$source_data$CryptoNetV1$CryptoNetV1_subnetwork_types) {
    cat(
        "Gathering CryptoNetV1 subentwork: ", subnetwork_type, " ... \n",
        sep = "")
...
CryptoNetV1_subnetworks <-
    ...
    purrr::map(
        .f = function(subnetwork_type) {
            readr::read_tsv(
                file =
                    parameters$source_data$CryptoNetV1$CryptoNetV1_subnetwork_path |>
                    stringr::str_replace("SUBNETWORK", subnetwork_type),
                col_names = c("target1", "target2", "score"),
