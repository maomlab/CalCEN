...
tasks <- tibble::tibble(
    ...
    dplyr::do({
        ...
        query <- .$sequence
        cat(
            "Submitting blastp task for batch '", batch, "' querying ",
            "'", length(query), "' genes against the genome\n", sep = "")
...
blastp_network_long <- blastp |>
    dplyr::transmute(
        ...
        rank_score =
            rank(bit_score, na.last = "keep", ties.method = "average") /
            dplyr::n()) |>
...
blastp_network_long |>
    ...
    readr::write_tsv(
        file = paste0(
            "product/blastp_network_long_", CalCEN::date_code(), ".tsv"))
