...
ggplot2::ggsave(
    filename = paste0(
        "product/crypto_coexp_of_sac_complexes/",
        "coexp_distribution_of_co-complex_genes_hist_",
...
# mean co-expression score of proteins in ribosomal protein complexes
sac_complex_interactions |>
    dplyr::filter(!is.na(h99_gene_id_1), !is.na(h99_gene_id_2)) |>
    dplyr::filter(
        complex_name |> stringr::str_detect("ribosomal") |
        complex_name |> stringr::str_detect("ribonucleoprotein")) |>
    dplyr::distinct(h99_gene_id_1, h99_gene_id_2, .keep_all = TRUE) |>
    dplyr::summarize(mean_cs = mean(coexp_score))
# 0.84

# mean co-expression score of proteins in non-ribosomal protein complexes
sac_complex_interactions |>
    dplyr::filter(!is.na(h99_gene_id_1), !is.na(h99_gene_id_2)) |>
    dplyr::filter(
        !(complex_name |> stringr::str_detect("ribosomal")) &
        !(complex_name |> stringr::str_detect("ribonucleoprotein"))) |>
    dplyr::distinct(h99_gene_id_1, h99_gene_id_2, .keep_all = TRUE) |>
    dplyr::summarize(mean_coexp = mean(coexp_score))
# 0.75
