...
todo_runs |>
    readr::write_tsv(
        file = paste0(
            parameters$data_paths$base_dir, "/",
            "intermediate_data/todo_runs_", tag, ".tsv"))
n_jobs <- nrow(todo_runs)

cat(
    "# Submitting ", n_jobs, " estimate expression jobs in parallel\n",
    sep = "")
...
if (cluster_type == "SGE") {
    ...
    cat(info_message, "\n", sep = "")
    cat(
        "Check results when done: intermediate_data/",
        "estimated_expression_", tag, "/logs\n",
        sep = "")
    ...
    cat(
        "Check results: intermediate_data/",
        "estimated_expression_", tag, "/logs\n", sep = "")
    ...
    todo_runs |> plyr::a_ply(1, function(ca_run) {
      ...
      work_dir <- paste0(
          parameters$data_paths$scratch_dir, "/estimated_expression_", tag, "/",
          "estimated_expression_", ca_run$run_accession)
      ...
      CalCEN::estimate_expression(
          ...
          results_dir = paste0(
              getwd(), "/intermediate_data/estimated_expression_", tag),
          logs_dir = paste0(
              getwd(), "/intermediate_data/estimated_expression_", tag, "/logs"),
          work_dir = work_dir,
    ...
    z <- data.frame(run_accession =
        list.files(
            paste0(
                getwd(), "/intermediate_data/",
                "estimated_expression_", tag, "/logs")) |>
        stringr::str_extract("^[^.]+")) |>
      dplyr::mutate(
          temp_results = paste0(
              getwd(), "/intermediate_data/estimated_expression_", tag, "/",
              run_accession, ".genes.results"),
          perm_results = paste0(
              getwd(), "/intermediate_data/estimated_expression/",
              run_accession, ".genes.results"),
          temp_log = paste0(
              getwd(), "/intermediate_data/estimated_expression_", tag, "/logs/",
              run_accession, ".log"),
          perm_log = paste0(
              getwd(), "/intermediate_data/estimated_expression/logs/",
              run_accession, ".log")) |>
