...
gdrive_path <- paste0(
		"~/Collaborations/Candida Functional Genomics/",
		"HSP90 Physical Interactors/Datasets/SILAC dataset/Ca-Sc-Sp")
raw_path <- "raw_data/silac/Ca-Sc_sp"
dir.create(raw_path, recursive = TRUE, showWarnings = FALSE)

#### load raw silac data from googledrive and
#### store it in the raw_data directory

googledrive::drive_download(
		file = paste0(
				gdrive_path, "/raw_data/",
				"Yeast_IEF_IEX_CA_msb_prot_count_uniqpeps_FDR0010_no_singletons.txt"),
		path = paste0(
				raw_path,
				"/Yeast_IEF_IEX_CA_msb_prot_count_uniqpeps_FDR0010_no_singletons.txt"))

ca_silac_profile <- readr::read_tsv(
		paste0(
				"raw_data/silac/Ca-Sc-Sp/",
				"Yeast_IEF_IEX_CA_msb_prot_count_uniqpeps_FDR0010_no_singletons.txt") |>
	dplyr::rename(uniprot_accn = ProteinID)

ca_silac_profile <- ca_silac_profile |> map_ca_uniprot_accn_to_feature_name()

##### complexes predicted by EPIC using  SILAC co-fractation,
##### seeded with Sac complexes and YeastNet
